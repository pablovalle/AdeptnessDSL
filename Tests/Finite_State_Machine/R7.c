...
int evaluatePreConditions_R7(int State, int Supported, int good) {
	return ( State == 2.0 && Supported == 1.0 && good == 1.0 ) ;
}
...
Verdict performEvaluation_R7(SensorInput *inputs, int inputQty, double timeStamp){
	...
	insertArray(&preconditionGiven,evaluatePreConditions_R7(State.array[cycle], Supported.array[cycle], good.array[cycle]));	
	if(preconditionGiven.array[cycle]==1){
	//Step 3: Sacar confidence. Si se da la precondicion (when: (Elevator1DoorStatus==1 && Elevator1DoorSensor == 1))
		insertArray(&conf,confCalculator_R7(State.array[cycle] ));
	}else{
		insertArray(&conf,2);
	}
	...
	verdict = checkGlobalVerdict_R7(conf, timeStampOracle); 
