...
int evaluatePreConditions_R12(int Senstate, int Request, int Limits) {
	return ( Senstate == 2.0 && Request == 0.0 && Limits == 0.0 ) ;
}
...
Verdict performEvaluation_R12(SensorInput *inputs, int inputQty, double timeStamp){
	...
	insertArray(&preconditionGiven,evaluatePreConditions_R12(Senstate.array[cycle], Request.array[cycle], Limits.array[cycle]));	
	if(preconditionGiven.array[cycle]==1){
	//Step 3: Sacar confidence. Si se da la precondicion (when: (Elevator1DoorStatus==1 && Elevator1DoorSensor == 1))
		insertArray(&conf,confCalculator_R12(Senstate.array[cycle] ));
	}else{
		insertArray(&conf,2);
	}
	...
	verdict = checkGlobalVerdict_R12(conf, timeStampOracle); 
