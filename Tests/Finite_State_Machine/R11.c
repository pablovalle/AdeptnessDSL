...
int evaluatePreConditions_R11(int Senstate, int Request) {
	return ( Senstate == 0.0 && Request == 0.0 ) ;
}
...
Verdict performEvaluation_R11(SensorInput *inputs, int inputQty, double timeStamp){
	...
	insertArray(&preconditionGiven,evaluatePreConditions_R11(Senstate.array[cycle], Request.array[cycle]));	
	if(preconditionGiven.array[cycle]==1){
	//Step 3: Sacar confidence. Si se da la precondicion (when: (Elevator1DoorStatus==1 && Elevator1DoorSensor == 1))
		insertArray(&conf,confCalculator_R11(Senstate.array[cycle] ));
	}else{
		insertArray(&conf,2);
	}
	...
	verdict = checkGlobalVerdict_R11(conf, timeStampOracle); 
