...
int main(){
    ...
    double EValveState[10]={0.033333333,3.1,5.4,10.8,15.025,10.9,17.4,11.46,13.3,9.92};
    double Tank2SensorHValue[10]={6,6,9,7,8,10,6,10,10,14};
	double Tank2SensorMValue[10] = { 6,6,9,7,8,10,6,10,10,14 };
	double EValveStatePrev[10] = { 6,6,9,7,8,10,6,10,10,14 };
	Verdict verdict;
    int emaitza[10];
    double conf[10];
    ...
    for(int i=0; i<10; i++){
        ...
        a.Tank2SensorHValue = Tank2SensorHValue[i];
		a.Tank2SensorMValue = Tank2SensorMValue[i];
		a.EValveStatePrev = EValveStatePrev[i];
        verdict=performEvaluation_R2_10_b(&a,1,timeStamp*i);
