...
Verdict performEvaluation_R2_9_a(SensorInput *inputs, int inputQty, double timeStamp){
	...
	insertArray(&preconditionGiven,evaluatePreConditions_R2_9_a(Tank2SensorHValue.array[cycle] ));	
	if(preconditionGiven.array[cycle]==1){
	//Step 3: Sacar confidence. Si se da la precondicion (when: (Elevator1DoorStatus==1 && Elevator1DoorSensor == 1))
		insertArray(&conf,confCalculator_R2_9_a(PValveState.array[cycle] ));
	}else{
		insertArray(&conf,2);
	}
	...
	verdict = checkGlobalVerdict_R2_9_a(conf, timeStampOracle); 
