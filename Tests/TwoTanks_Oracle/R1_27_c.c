...
Verdict performEvaluation_R1_27_c(SensorInput *inputs, int inputQty, double timeStamp){
	...
	insertArray(&preconditionGiven,evaluatePreConditions_R1_27_c(Valve_State.array[cycle], PValveState.array[cycle], EValveState.array[cycle] ));	
	if(preconditionGiven.array[cycle]==1){
	//Step 3: Sacar confidence. Si se da la precondicion (when: (Elevator1DoorStatus==1 && Elevator1DoorSensor == 1))
		insertArray(&conf,confCalculator_R1_27_c(Tank2HeightPrev.array[cycle], Tank1ValveFlow.array[cycle], Tank2EValveFlow.array[cycle], TimeIncrement.array[cycle], Tank2CrossSec.array[cycle], Tank2Height.array[cycle] ));
	}else{
		insertArray(&conf,2);
	}
	...
	verdict = checkGlobalVerdict_R1_27_c(conf, timeStampOracle); 
...
double confCalculator_R1_27_c(double Tank2HeightPrev,double Tank1ValveFlow,double Tank2EValveFlow,double TimeIncrement,double Tank2CrossSec,double signal){
	double conf=0;
	if(signal== ( Tank2HeightPrev+ ( ( ( Tank1ValveFlow- Tank2EValveFlow) * TimeIncrement) / Tank2CrossSec) ) ){
		conf=1;
	}
	else if(signal< ( Tank2HeightPrev+ ( ( ( Tank1ValveFlow- Tank2EValveFlow) * TimeIncrement) / Tank2CrossSec) ) ){
		conf=(signal- ( Tank2HeightPrev+ ( ( ( Tank1ValveFlow- Tank2EValveFlow) * TimeIncrement) / Tank2CrossSec) ) )/( ( Tank2HeightPrev+ ( ( ( Tank1ValveFlow- Tank2EValveFlow) * TimeIncrement) / Tank2CrossSec) ) -(-99999));
	}
	else if(signal> ( Tank2HeightPrev+ ( ( ( Tank1ValveFlow- Tank2EValveFlow) * TimeIncrement) / Tank2CrossSec) ) ){
		conf=( ( Tank2HeightPrev+ ( ( ( Tank1ValveFlow- Tank2EValveFlow) * TimeIncrement) / Tank2CrossSec) ) -signal)/(9999- ( Tank2HeightPrev+ ( ( ( Tank1ValveFlow- Tank2EValveFlow) * TimeIncrement) / Tank2CrossSec) ) );
	}
	return conf;
}
