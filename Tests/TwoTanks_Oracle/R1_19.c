...
int evaluatePreConditions_R1_19(int Pump_State, int Valve_State) {
	return ( Pump_State == 0.05 && Valve_State == 1.0 ) ;
}
...
Verdict performEvaluation_R1_19(SensorInput *inputs, int inputQty, double timeStamp){
	...
	insertArray(&preconditionGiven,evaluatePreConditions_R1_19(Pump_State.array[cycle], Valve_State.array[cycle] ));	
	if(preconditionGiven.array[cycle]==1){
	//Step 3: Sacar confidence. Si se da la precondicion (when: (Elevator1DoorStatus==1 && Elevator1DoorSensor == 1))
		insertArray(&conf,confCalculator_R1_19(Tank1HeightPrev.array[cycle], Tank1Height.array[cycle] ));
	}else{
		insertArray(&conf,2);
	}
	...
	verdict = checkGlobalVerdict_R1_19(conf, timeStampOracle); 
...
double confCalculator_R1_19(double Tank1HeightPrev,double signal){
	...
	if(signal< Tank1HeightPrev){
		conf= ( Tank1HeightPrev-signal)/( Tank1HeightPrev-(-99999));
	}
	else{
		conf= ( Tank1HeightPrev-signal)/(99999- Tank1HeightPrev);
	}
	return conf;
}
