...
Verdict performEvaluation_R1_10(SensorInput *inputs, int inputQty, double timeStamp){
	...
	insertArray(&preconditionGiven,evaluatePreConditions_R1_10(Tank2Height.array[cycle], Tank2SensorHHeight.array[cycle] ));	
	if(preconditionGiven.array[cycle]==1){
	//Step 3: Sacar confidence. Si se da la precondicion (when: (Elevator1DoorStatus==1 && Elevator1DoorSensor == 1))
		insertArray(&conf,confCalculator_R1_10(Tank2SensorHValue.array[cycle] ));
	}else{
		insertArray(&conf,2);
	}
	...
	verdict = checkGlobalVerdict_R1_10(conf, timeStampOracle); 
