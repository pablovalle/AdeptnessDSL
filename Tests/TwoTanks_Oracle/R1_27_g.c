...
int evaluatePreConditions_R1_27_g(int Valve_State, int PValveState, int EValveState) {
	return ( Valve_State == 0.0 && PValveState == 0.0 && EValveState == 0.0 ) ;
}
...
Verdict performEvaluation_R1_27_g(SensorInput *inputs, int inputQty, double timeStamp){
	...
	insertArray(&preconditionGiven,evaluatePreConditions_R1_27_g(Valve_State.array[cycle], PValveState.array[cycle], EValveState.array[cycle] ));	
	if(preconditionGiven.array[cycle]==1){
	//Step 3: Sacar confidence. Si se da la precondicion (when: (Elevator1DoorStatus==1 && Elevator1DoorSensor == 1))
		insertArray(&conf,confCalculator_R1_27_g(Tank2HeightPrev.array[cycle], Tank2Height.array[cycle] ));
	}else{
		insertArray(&conf,2);
	}
	...
	verdict = checkGlobalVerdict_R1_27_g(conf, timeStampOracle); 
...
double confCalculator_R1_27_g(double Tank2HeightPrev,double signal){
	...
	else if(signal< Tank2HeightPrev){
		conf=(signal- Tank2HeightPrev)/( Tank2HeightPrev-(-99999));
	}
	else if(signal> Tank2HeightPrev){
		conf=( Tank2HeightPrev-signal)/(9999- Tank2HeightPrev);
	}
	return conf;
}
