...
int evaluatePreConditions_R2_10_b(int Tank2SensorHValue, int Tank2SensorMValue, int EValveStatePrev) {
	return ( Tank2SensorHValue == 0.0 && Tank2SensorMValue == 1.0 && EValveStatePrev == 1.0 ) ;
}
Verdict evaluatePostConditions_R2_10_b(Verdict verdict, SensorInput *inputs, int inputQty) {
    verdict.verdict = VERDICT_PASSED;
    verdict.confidence = 1;
    return verdict;
}
Verdict performEvaluation_R2_10_b(SensorInput *inputs, int inputQty, double timeStamp){
    Verdict verdict;
	...
	insertArray(&preconditionGiven,evaluatePreConditions_R2_10_b(Tank2SensorHValue.array[cycle], Tank2SensorMValue.array[cycle], EValveStatePrev.array[cycle]));	
	if(preconditionGiven.array[cycle]==1){
	//Step 3: Sacar confidence. Si se da la precondicion (when: (Elevator1DoorStatus==1 && Elevator1DoorSensor == 1))
		insertArray(&conf,confCalculator_R2_10_b(EValveState.array[cycle] ));
	}else{
		insertArray(&conf,2);
	}
	...
	verdict = checkGlobalVerdict_R2_10_b(conf, timeStampOracle); 
	verdict.confidence=conf.array[cycle];
	
    return verdict;
}

double confCalculator_R2_10_b(double signal){
	double conf=0;
	if(signal==1.0){
		conf=1;
	}
...
Verdict checkGlobalVerdict_R2_10_b(Array conf, Array timeStampOracle){
	Verdict verdict;
