...
int evaluatePreConditions_R2_8_a(int Tank2SensorLValue, int Tank2SensorMValue) {
	return ( Tank2SensorLValue == 1.0 && Tank2SensorMValue == 0.0 ) ;
}
...
Verdict performEvaluation_R2_8_a(SensorInput *inputs, int inputQty, double timeStamp){
	...
	insertArray(&preconditionGiven,evaluatePreConditions_R2_8_a(Tank2SensorLValue.array[cycle], Tank2SensorMValue.array[cycle] ));	
	if(preconditionGiven.array[cycle]==1){
	//Step 3: Sacar confidence. Si se da la precondicion (when: (Elevator1DoorStatus==1 && Elevator1DoorSensor == 1))
		insertArray(&conf,confCalculator_R2_8_a(PValveState.array[cycle] ));
	}else{
		insertArray(&conf,2);
	}
	...
	verdict = checkGlobalVerdict_R2_8_a(conf, timeStampOracle); 
